...
$(document).ready(function () {
        ...
        show: function (overNode, iCateNo) {
            ...
            if (methods.aSubCategory[iCateNo] != undefined) {
                ...
                $(methods.aSubCategory[iCateNo]).each(function () {
                    ...
                    if (methods.aSubCategory[sNextParentNo] != undefined) aHtml.push('<a href="/product/list.html' + this.param + '" class="view"></a>'); // 190605 수정
    ...
    var offCover = {
        ...
        layout: function () {
            if ($("html").hasClass("expand")) {
                $("#btnFoldLayout").show();
                $(".side-cate").css({ visibility: "visible", display: "block" });
                $("html, body").css({ "overflow-x": "" });

                setTimeout(function () {
                    $("#btnFoldLayout").css({ background: "rgba(0,0,0,0.7)" });
                    $(".side-cate").css({ "z-index": 200 });
                }, 350);
            } else {
                $("#btnFoldLayout").hide();
                $(".side-cate").css({ "z-index": 0 });
                setTimeout(function () {
                    $(".side-cate").css({ visibility: "", display: "none" });
                }, 300);
            }
            $(".side-cate").css({ visibility: "visible" });
        },
        resize: function () {
    ...
    $("#header .fold, .side-cate .btnClose").click(function (e) {
        $("#dimmedSlider").toggle();
    ...
    $("#slideCateList li > a.cate").click(function (e) {
        ...
        if ($(this).parent().attr("class") == "xans-record- selected") {
            ...
            if (!iCateNo) return;
            $(".side-cate #slideCateList li").removeClass("selected");
            methods.close();
            methods.show(this.parentNode, iCateNo);
        }
    });

    $(".side-cate ul a.cate").click(function (e) {
